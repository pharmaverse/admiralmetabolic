...
## Merge ADVS and ADSL variables {#merge}
In this section, we will merge relevant variables from the ADVS (Analysis Dataset for Vital Signs) and ADSL (Analysis Dataset for Subject-Level) datasets into the ADLB dataset. This is necessary to include variables such as BMI, Waist Circumference, Age, Sex, and T2DM status, which are required for the derivation of metabolic parameters and scores in subsequent sections.
...
# Merge AGE, SEX, and T2DM from ADSL to ADLB
adlb <- adlb %>%
  derive_vars_merged(
...
In this section, we will demonstrate how to derive the metabolic parameters and scores using the dedicated functions provided by the `admiralmetabolic` package.
