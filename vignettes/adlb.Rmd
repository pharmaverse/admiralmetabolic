...
param_lookup <- tibble::tribble(
  ...
  "TRIG", "TRIG", "Triglycerides (mg/gL)", 9
)
```
...
adlb <- lb %>%
 ...
 adlb <- adlb %>%
  ...
  mutate(
    PARCAT1 = LBCAT,
    PARCAT2 = case_when(
      LBTESTCD %in% c("TRIG", "INSULIN") ~ "CV",
      TRUE ~ "SI"
    ),
    # Assign AVAL based on LBORRES for specific tests, otherwise use LBSTRESN
    AVAL = case_when(
      LBTESTCD %in% c("TRIG", "INSULIN") ~ as.numeric(LBORRES),
      TRUE ~ LBSTRESN
    ),
    # Only populate AVALC if character value is non-redundant with AVAL, except for TRIG and INSULIN
    AVALC = case_when(
      LBTESTCD %in% c("TRIG", "INSULIN") ~ NA_character_,
      is.na(AVAL) | as.character(AVAL) != LBSTRESC ~ LBSTRESC, # Compare AVAL with LBSTRESC
      TRUE ~ NA_character_
    ),
    ANRLO = LBSTNRLO,
    ANRHI = LBSTNRHI
...
adlb <- adlb %>%
  derive_param_computed(
    ...
    set_values_to = exprs(
      AVAL = if_else(
        complete.cases(AVAL.INSULIN, AVAL.GLUC),
        (AVAL.INSULIN * AVAL.GLUC) / 22.5,
        NA_real_
      ),
      PARAMCD = "HOMAIR",
...
adlb <- adlb %>%
  derive_param_computed(
    ...
    set_values_to = exprs(
      AVAL = if_else(
        complete.cases(AVAL.TRIG, BMI, AVAL.GGT, WSTCIR),
        {
