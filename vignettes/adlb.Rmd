...
For the derivation of Glucose and Insulin, only fasted results (where LBFAST is 'Y') are considered. For all other laboratory tests, the fasting status does not affect their inclusion in the dataset at this stage.

```{r}
adlb <- lb %>%
  # Keep only fasted results for GLUC and INSULIN, keep all others
  filter(if_else(LBTESTCD %in% c("GLUC", "INSULIN"), LBFAST == "Y", TRUE)) %>%
