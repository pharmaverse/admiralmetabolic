...
## Convert Lab Results {#convert}
Some laboratory parameters are recorded in units that are not suitable for direct use in metabolic score calculations (e.g., FLI, HOMA-IR). This section demonstrates how to convert the units of relevant parameters (Triglycerides and Insulin) to the required units for subsequent derivations.
```{r}
# Convert units for specific parameters
adlb_converted <- adlb %>%
  filter(PARAMCD %in% c("TRIG", "INSULIN")) %>%
  mutate(
    AVAL = case_when(
...
## Merge ADVS variables {#merge}
In this section, we will merge relevant variables from the ADVS (Analysis Dataset for Vital Signs) dataset into the ADLB dataset. This is necessary to include variables such as BMI and Waist Circumference, which are required for the derivation of FLI score parameter in subsequent section.
...
dataset_vignette(
  ...
  display_vars = exprs(!!!get_admiral_option("subject_keys"), PARAMCD, PARAM, AVAL, ADY, AVISIT, AVISITN, BMI, WSTCIR)
)
```

## Derive Metabolic Parameters {#derive_metabolic}

In this section, we will derive the metabolic parameters and scores described previously by directly implementing the calculation formulas.

### Homeostasis Model Assessment – Insulin Resistance (HOMA-IR)

This part describes how to calculate the HOMA-IR index from fasting plasma glucose and fasting plasma insulin. This derivation does not require merging data from other datasets.

```{r}
# Derive HOMA-IR using derive_param_computed
adlb_homair <- adlb1 %>%
  derive_param_computed(
    by_vars = exprs(STUDYID, USUBJID, AVISIT, AVISITN, ADT, ADY, !!!adsl_vars),
    parameters = c("INSULMIU", "GLUC"),
    set_values_to = exprs(
      AVAL = case_when(
        !is.na(AVAL.INSULMIU) & !is.na(AVAL.GLUC) ~ (AVAL.INSULMIU * AVAL.GLUC) / 22.5
      ),
      PARAMCD = "HOMAIR",
  ...
  filter(PARAMCD %in% c("HOMAIR"))
```

```{r echo=FALSE}
# Example display of derived data
dataset_vignette(
  arrange(adlb_homair, USUBJID, ADY, PARAMN),
...
adlb_fli <- adlb1 %>%
  derive_param_computed(
    by_vars = exprs(STUDYID, USUBJID, AVISIT, AVISITN, ADT, ADY, BMI, WSTCIR, !!!adsl_vars),
    parameters = c("TRIGMG", "GGT"),
    set_values_to = exprs(
      AVAL = case_when(
        !is.na(AVAL.TRIGMG) & !is.na(BMI) & !is.na(AVAL.GGT) & !is.na(WSTCIR) ~ {
          lambda <- 0.953 * log(AVAL.TRIGMG) + 0.139 * BMI + 0.718 * log(AVAL.GGT) + 0.053 * WSTCIR - 15.745
  ...
  filter(PARAMCD %in% c("FLI"))
```

```{r echo=FALSE}
# Example display of derived data
dataset_vignette(
  arrange(adlb_fli, USUBJID, ADY, PARAMN),
  display_vars = exprs(!!!get_admiral_option("subject_keys"), PARAMCD, PARAM, AVAL, ADY, AVISIT, AVISITN)
)
```

Combine derived parameters with original ADLB data:

```{r}
# Combine derived parameters with original ADLB data
adlb <- bind_rows(adlb, adlb_homair, adlb_fli) %>%
  arrange(STUDYID, USUBJID, ADT, PARAMN) # Arrange for consistency
```
