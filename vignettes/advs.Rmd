...
Within this vignette, the `DM` is used as the basis for `ADSL`:
...
The function `derive_vars_dt()` can be used to derive `ADT`.
...
Next step is to assign parameter level values such as `PARAMCD`, `PARAM`,
`PARAMN`, `PARCAT1`, etc.,and for this a lookup can be created based on
the SDTM `--TESTCD` value to join to the source data.
The only difference here would be the the additional parameters
collected in metabolic trials - `BMI`, `HIPCIR` and `WSTCIR`.
...
The mapping of `AVAL` is left to the ADaM programmer. An example mapping may be:
...
In metabolic trials, `BMI` is often calculated at source using collected
height and weight for screening purposes. But while creating the `ADVS`
...
=======
In this step, we create parameter Body Mass Index (`BMI`) for the `ADVS`
>>>>>>> 4b1c85d (#22 : Resolved comments and no hidden codes)
...
Next step is to derive the analysis baseline flag `ABLFL` using the `{admiral}`
...
Lastly, the `BASE` can be derived using the `{admiral}` function
`derive_var_base()`. Example:
...
In this step, we derive change and percent change from baseline using the `{admiral}` 
...
`PCHG` is `(AVAL - BASE) / absolute value (BASE) * 100`.  Example calls are:
...
5%, ≥ 10%, or ≥ 15% from baseline to end of treatment or a certain visit,
