...
derive the usual ADVS analysis variables. The `{admiral}` [Creating a
BDS Finding ADaM vignette](https://pharmaverse.github.io/admiral/articles/bds_finding.html)
...
collected in metabolic trials - `BMI`, `HIPCIR` and `WSTCIR`. Next step
is to assign parameter level values such as `PARAMCD`, `PARAM`,
...
param_lookup <- tribble(
  ...
  "BMI", "Body Mass Index", "Body Mass Index(kg/m^2)", 3, "Subject Characteristic", 1,
...
In this step, we create parameter `Body Mass Index` (BMI) for `ADVS`
...
5%, ≥ 10%, or ≥ 15% from baseline to end of treatment or certain visit,
is a common endpoint in obesity trials. To accommodate these criteria,
